...
{
    //[HarmonyPatch]
    static class RPGStyleInventory_PopupMenu_SaleOfGoodsPatch
    {
    ...
    static class RPGStyleInventory_DrawSlotIcons_SaleOfGoodsPatch
    {
        internal static void Postfix(object __instance, Thing thing, bool equipment, bool inventory, Rect slotRect, ref float x, ref float y)
        {
            ...
            {
                RPGStyleInventory_SaleOfGoodsPatch.LDrawSlotIcon.Invoke(__instance, new object[] { slotRect, x, y, NUPUtility.texStripThing, (string)"StripThing".Translate() });
            }
        }
    }

    [StaticConstructorOnStartup]
    static class RPGStyleInventory_SaleOfGoodsPatch
    {
        ...
        static RPGStyleInventory_SaleOfGoodsPatch()
        {
            var harmonyInstance = new Harmony("net.avilmask.rimworld.mod.SaleOfGoods.RPGStyleInventory");
            Type type;
            if ((type = AccessTools.TypeByName("Sandy_Detailed_RPG_GearTab")) != null)
            {
                ...
                HarmonyMethod hm = new HarmonyMethod(typeof(ITab_Pawn_Gear_DrawThingRow_SaleOfGoodsPatch), nameof(ITab_Pawn_Gear_DrawThingRow_SaleOfGoodsPatch.Prefix), null);
                harmonyInstance.Patch(mi, hm, null);

                mi = AccessTools.Method(type, "PopupMenu", null, null);
                if (mi != null)
                {
                    hm = new HarmonyMethod(typeof(RPGStyleInventory_PopupMenu_SaleOfGoodsPatch), nameof(RPGStyleInventory_PopupMenu_SaleOfGoodsPatch.Postfix), null);
                ...
                if (mi != null)
                {
                    hm = new HarmonyMethod(typeof(RPGStyleInventory_DrawSlotIcons_SaleOfGoodsPatch), nameof(RPGStyleInventory_DrawSlotIcons_SaleOfGoodsPatch.Postfix));
                    harmonyInstance.Patch(mi, null, hm);
                }
            }
        }
    ...
    [StaticConstructorOnStartup]
    static class AwesomeInventory_SaleOfGoodsPatch
    {
         static AwesomeInventory_SaleOfGoodsPatch()
        {
            var harmonyInstance = new Harmony("net.avilmask.rimworld.mod.SaleOfGoods.AwesomeInventory");
            Type type;
            if ((type = AccessTools.TypeByName("AwesomeInventory.UI.DrawGearTabWorker")) != null)
            {
                var mi = AccessTools.Method(type, "DrawThingRow", null, null);
                HarmonyMethod hm = new HarmonyMethod(typeof(AwesomeInventory_SaleOfGoodsPatch), nameof(AwesomeInventory_SaleOfGoodsPatch.Prefix), null);
                harmonyInstance.Patch(mi, hm, null);
            }
        }
