...
namespace SaleOfGoods.Patches
{
    ...
    static class PawnRenderer_RenderPawnInternal_SaleOfGoodsPatch
    {
        static void renderStaticEquipment(Pawn pawn, Rot4 bodyFacing, Vector3 vector, Quaternion quaternion, PawnRenderFlags flags)
        {
            if (SaleOfGoodsSettings.drops || flags.FlagSet(PawnRenderFlags.Portrait) || !pawn.Dead || pawn.Spawned || pawn.equipment == null || pawn.equipment.Primary == null) return;
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il, MethodBase mb)
        {
            ...
            object label = null;
            MethodInfo LrenderStaticEquipment = AccessTools.Method(typeof(PawnRenderer_RenderPawnInternal_SaleOfGoodsPatch), nameof(PawnRenderer_RenderPawnInternal_SaleOfGoodsPatch.renderStaticEquipment));
